...
public class ClimbIOSim implements ClimbIO {
  ...
  private AnalogEncoderSim _leftEncoderSim =
      new AnalogEncoderSim(new AnalogEncoder(Constants.AIO.CLIMB_LEFT_SENSOR));
  private AnalogEncoderSim _rightEncoderSim =
      new AnalogEncoderSim(new AnalogEncoder(Constants.AIO.CLIMB_RIGHT_SENSOR));

  private SolenoidSim _leftSolenoidSim =
      new SolenoidSim(Constants.Pnuematics.MODULE_TYPE, Constants.Pnuematics.SOLENOID_LEFT);
  private SolenoidSim _rightSolenoidSim =
      new SolenoidSim(Constants.Pnuematics.MODULE_TYPE, Constants.Pnuematics.SOLENOID_RIGHT);
  ...
  public ClimbIOSim() {
    ...
    MechanismLigament2d lig =
        robot.append(new MechanismLigament2d("lig", 1, 90, 20, new Color8Bit(Color.kOrange)));

    _climb =
        lig.append(new MechanismLigament2d("SubClimb", 0.5, 45, 10, new Color8Bit(Color.kOrange)));
  ...
  public void updateInputs(ClimbIOInputs inputs) {
    ...
    double leftDelta =
        CLIMB_SENSOR_RATE_DEG_PER_SEC
            * Constants.General.LOOP_PERIOD_SECS
    ...
    _leftEncoderSim.setPosition(
        _leftEncoderSim.getPosition().plus(Rotation2d.fromDegrees(leftDelta)));
    _rightEncoderSim.setPosition(
        _rightEncoderSim.getPosition().plus(Rotation2d.fromDegrees(rightDelta)));
  ...
  public void setVoltageLeft(double volts) {
    _leftAppliedVolts =
        MathUtil.clamp(volts, -MOTOR_VOLTAGE_LIMIT, MOTOR_VOLTAGE_LIMIT); // Figure out how this
